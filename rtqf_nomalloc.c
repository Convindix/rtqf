...
void pollardRho(uint64 n, uint64* factors){
	int maxLength = intLog(n);
	int i = 0;
	if(maxLength == 64){
		maxLength = 63;
	}
...
int isSumOfTwoSqrs(uint64 n, uint64* factors){
...
int isInTQF(uint64 N, uint64* factors){ //N of the form x**2 + y**2 + 10*z**2
	uint64 z = 0;
	...
	while(tenZSquared <= N){
		if(isSumOfTwoSqrs(N - tenZSquared, factors)){
			return 1;
		}
		tenZSquared += 20*z + 10;
		z++;
...
int main(){
	...
	while(N < MAXSAFE){ //Every 10k+5 is in TQF, so unrolled loop that avoids 5 mod 10
		...
		N += 2;
		if(N % 1024 == 1){
			printf("Conjecture holds up to %llu\n", N);
		}
	};
